        ...
        public async Task<IActionResult> Create([Bind("Name,Email,Phone,Address,Region")] Client client)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        _logger.LogInformation("Client created: {ClientName} with ID: {ClientId}", createdClient.Name, createdClient.ClientId);
                        TempData["Success"] = $"Client '{createdClient.Name}' was created successfully!";
                        return RedirectToAction(nameof(Index));
                    }
                    else
                    {
                        _logger.LogWarning("Failed to create client - API returned null");
                        ModelState.AddModelError("", "Failed to create client. Please try again.");
                    }
                }
                catch (HttpRequestException ex)
                {
                    _logger.LogError(ex, "API connection error while creating client");
                    ModelState.AddModelError("", "Cannot connect to the API. Please ensure the API is running.");
                }
                catch (Exception ex)
                {
